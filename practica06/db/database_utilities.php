<?php
	...
	//Funcion que permite agregar un nuevo usuario a la base de datos, requiere nombre y correo.
	function addUser($email,$password,$status,$tipo){
		global $db;
		$db = getPDO();
		$stmt = $db->prepare("INSERT INTO user (email,password,status_id,user_type_id) 
								VALUES (:email,:password,:status,:tipo)");
	...
	//Funcion que permite actualizar los atributos de un usuario existente en la tabla 'user'.
	function updateUser($id,$email,$password,$status_id,$type_id){
		global $db;
		$db = getPDO();
		$stmt = $db->prepare("UPDATE user SET email=:email,password=:password,status_id=:status_id,user_type_id=:type_id where id=:id");
		$stmt->bindParam(':id', $id);
		$stmt->bindParam(':email', $email);
		$stmt->bindParam(':password', $password);
	...
	//Funcion que permite eliminar un usuario de la base de datos utilizando su id, pero ahora de la tabla 'user'.
	...
	//Funcion que permite realizar una busqueda de un usuario de la tabla 'user' para obtener todos sus atributos mediante su id.
	...
	function addLog($id_user){
		...
		$db = getPDO();
		$stmt = $db->prepare("INSERT INTO user_log(date_logged_in, user_id)  VALUES (:date_log, :user_id) ");
		$stmt->bindParam(':date_log', date("Y-m-d"));
